...
SELECT ROUND(AVG(int_rate)*100,4)  AS Avg_Int_Rate FROM bank_loan_data


--MTD Average Interest
SELECT ROUND(AVG(int_rate)*100,4)  AS MTD_Avg_Int_Rate FROM bank_loan_data
WHERE MONTH(issue_date) = 12


--PMTD Average Interest
SELECT ROUND(AVG(int_rate)*100,4) AS PMTD_Avg_Int_Rate FROM bank_loan_data
WHERE MONTH(issue_date) = 11
...
--PMTD Avg DTI
SELECT ROUND(AVG(dti)*100,4) AS PMTD_Avg_DTI FROM bank_loan_data
WHERE MONTH(issue_date) = 11
...
--1__/*See the results when we hit the Grade A in the filters for dashboards. */
SELECT 
	purpose AS PURPOSE, 
	COUNT(id) AS Total_Loan_Applications,
	SUM(loan_amount) AS Total_Funded_Amount,
